...
chmod 755 -R $WORK_HOME/bin
$JAVA_HOME/bin/java -Xms128M -Xmx400M -XX:PermSize=64M -XX:MaxPermSize=128M -Ddisconf.env=${envType} -DENV_TYPE=${envType} -DHOSTNAME=`hostname -i` -DIS_CONTAINER=1 -classpath $CLASSPATH com.qingqing.rrw.LiveRrwApplication $@
